...
Location = {
  ...
  /**
   * Grab path or hash as route path
   *
   * @api private
   */
  hashOrPath: function() {
      if(location.hash) {
          return location.hash.replace('#','').replace(/^[a-zA-Z]/,function(capture) {
              return '/'+capture;
          }) + location.search;
      }
      else
  ...
  start: function () {
    ...
    self.isStarted = true;
    self.replaceState({}, null, self.hashOrPath());
  },
  ...
  onPopState: function (e) {
    this._setState(e.state, null, this.hashOrPath());
  }
};
